...
$$\begin{eqnarray}
    ...
    \begin{cases}
      ...
      \frac{1}{T}sinc\left( \frac{t}{T} \right)\frac{cos\left( \frac{\pirt}{T}\right)}{1-\left( \frac{2rt}{T} \right)^2}, & \text{otherwise}
    \end{cases}
\end{eqnarray}$$
...
It is shown in the text section 3.2.2 that for a binary signal in a gaussian noise channel, this can be further simplified to $$max(SNR) = \frac{2E_b}{N_0}$$ (text eqn. 3.52).
...
where $$Q$$ is the Q-function  Considering the relation between $$SNR$$ and $$\frac{E_b}{N_0}$$ the above can be rewritten as

$$
BER = Q\left( \sqrt{SNR} \right).
$$

So by measuring the SNR of a BPSK signal you can generate a $$BER-\frac{E_b}{N_0}$$ curve.
